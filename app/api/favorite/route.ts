...
async function GET(req: NextRequest) {
  ...
  try {
    if (req.nextUrl.searchParams.get('id')) {
      const favorite = await Favorite.findById(req.nextUrl.searchParams.get('id'))
        .populate('user')
        .populate('novel');
      if (!favorite) {
...
async function DELETE(req: NextRequest) {
  ...
  try {
    const deletedFavorite = await Favorite.findByIdAndDelete(req.nextUrl.searchParams.get('id'));
    if (!deletedFavorite) {
      return NextResponse.json({ error: 'Favorite not found' }, { status: 404 });
    }
