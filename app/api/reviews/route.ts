...
async function GET(req: NextRequest) {
  ...
  try {
    if (req.nextUrl.searchParams.get('id')) {
      const review: IReview | null = await Review.findById(req.nextUrl.searchParams.get('id'))
...
async function DELETE(req: NextRequest) {
  ...
  try {
    const review: IReview | null = await Review.findByIdAndDelete(req.nextUrl.searchParams.get('id'));
    if (!review) {
      return NextResponse.json({ error: 'Review not found' }, { status: 404 });
    }
