...
const NovelForm: React.FC<NovelFormProps> = ({ initialData, onSubmit }) => {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ maxWidth: '600px', margin: 'auto' }}>
      ...
      <FormInput label="Description" name="description" value={formData.description} onChange={handleChange} required type="textarea" />
      <FormInput label="Release Date" name="releaseDate" value={formData.releaseDate} onChange={handleChange} required type="date" />
      <FormInput label="Cover Image URL" name="coverImage" value={formData.coverImage} onChange={handleChange} />
      <FormInput label="Rating" name="rating" value={formData.rating.toString()} onChange={handleChange} required type="number" min="0" max="5" />
      ...
      <AuthorSelect
        ...
        onChange={(selectedOptions, actionMeta) => handleSelectChange(selectedOptions, 'authors')}
      ...
      <PublisherSelect
        ...
        onChange={(selectedOptions, actionMeta) => handleSelectChange(selectedOptions, 'publisher')}
      ...
      <GenreSelect
        ...
        onChange={(selectedOptions, actionMeta) => handleSelectChange(selectedOptions, 'genres')}
        onCreateOption={(inputValue) => handleCreateOption(inputValue, 'genres')}
      />
      <Spacer y={1} />
      <Button type="submit">{formData.id ? 'Update Novel' : 'Add Novel'}</Button>
    </form>
  );
};
