      ...
      <CardFooter className="justify-items-start py-8 gap-4">
        <div className="flex flex-col items-start">
          <Tooltip content={novel.title} isOpen={isHovered}>
            <h3 className="text-lg font-bold text-default-800 truncate max-w-full">{novel.title}</h3>
          </Tooltip>
          <p className="text-sm font-medium text-default-600">
            by {novel.authors.map((author) => 'name' in author && author.name).join(', ')}
          </p>
          <p className="text-sm font-medium text-default-600">Rating: {novel.rating}</p>
          <p className="text-sm font-medium text-default-600">
